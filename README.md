...
**Conclusion**: While the LGBM is computationally cheaper and slightly more precise, the GAT model offers higher recall (catching more toxic drugs) and provides superior structural interpretability via attention maps, which is crucial for chemists. So the tradeoff for these models is computation vs insight, as the performance is comparable.
